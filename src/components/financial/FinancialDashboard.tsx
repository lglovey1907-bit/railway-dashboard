...
// Features: unit toggle (Cr/Lacs), editable headers, add row, column visibility
// ─────────────────────────────────────────────────────────────────────────────
    ...
    <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4"
      ...
      <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl p-5 space-y-4"
        ...
        <div className="flex items-center justify-between">
          <p className="text-sm font-bold text-slate-900">Add Revenue Row</p>
        ...
        <div className="space-y-3">
          ...
          <div>
            <label className="text-[11px] font-semibold text-slate-600 mb-1 block">Short Code</label>
            <input value={code} onChange={e => setCode(e.target.value)}
              maxLength={6} placeholder="e.g. PLAT"
              className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-rail-400"/>
          </div>
          <div>
            <label className="text-[11px] font-semibold text-slate-600 mb-1 block">Colour</label>
...
export function FinancialDashboard({
  ...
  const records  = useMemo(() => store.getPublishedRecords(selectedFYId),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [selectedFYId, store.monthlyRecords]);
  const allRecords = useMemo(() => store.getAllRecords(selectedFYId),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [selectedFYId, store.monthlyRecords]);
  const rows = useMemo(() => buildCumulativeRows(records, store.revenueHeads, selectedFYId, selectedMonth),
    ...
    <div className={cn('flex flex-col gap-3', isCompact ? 'text-xs' : 'text-sm')}>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {selectedFY?.isCurrent && (
          <span className="px-2 py-0.5 bg-emerald-100 text-emerald-700 rounded-full text-[10px] font-semibold">● Live</span>
        )}
        ...
        <button onClick={toggleUnit}
          ...
          ₹&nbsp;{unit === 'cr' ? 'Crore' : 'Lacs'}
          <span className="text-[9px] opacity-60">{unit === 'cr' ? '→ Lacs' : '→ Cr'}</span>
        </button>
      ...
      {view === 'table' ? (
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
          ...
          <div className="px-3 py-2 bg-slate-50 border-t border-slate-100 text-[9px] text-slate-400 flex items-center gap-2 flex-wrap">
            ...
            {canManage && <span className="text-rail-500">· Double-click any header or row name to rename</span>}
